...
//   '.exe', '.dll', '.so', '.dylib',
//   '.lock', '.log', '.vsix', 
//   '.claude','.md', '.development','.production'
// ];
    ...
    async () => {
      ...
      if (selection === 'Select from Open Workspace') {
        ...
        if (folderUri && folderUri.length > 0) {
          ...
          const additionalIgnores = await vscode.window.showInputBox({
            prompt: 'Enter folders to ignore (comma-separated), or press Enter for default',
            placeHolder: 'e.g., coverage,.cache,tmp'
          });

          if (additionalIgnores?.trim()) {
            customIgnoreDirs = additionalIgnores.split(',').map(s => s.trim()).filter(Boolean);
          }
        }
      }
      ...
      const exportUri = await vscode.window.showSaveDialog({
        saveLabel: 'Export Project',
        defaultUri: vscode.Uri.file(path.join(projectRoot, fileName)),
        filters: { 'Claude Project Export': ['claude'] }
      });

      if (!exportUri) return;

      let exportPath = exportUri.fsPath;
      if (!exportPath.endsWith('.claude')) exportPath += '.claude';
      ...
      try {
        await vscode.window.withProgress({
          ...
          await exportProjectToFile(projectRoot!, exportPath, [...DEFAULT_IGNORE_DIRS, ...customIgnoreDirs], progress);
        });
...
async function exportProjectToFile(
  ...
  async function generateProjectStructure(rootDir: string): Promise<string> {
    ...
    async function traverseDir(dir: string, prefix = '') {
      const entries = await fsPromises.readdir(dir, { withFileTypes: true });
      entries.sort((a, b) => a.name.localeCompare(b.name));
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        if (ignoreDirs.some(ignoredDir => relativePath.split(path.sep).includes(ignoredDir))) {
          continue;
        }
        ...
        if (entry.isDirectory()) {
          structure.push(`${prefix}${currentPrefix}${entry.name}/`);
          await traverseDir(fullPath, prefix + (isLast ? '    ' : '│   '));
        } else {
          const ext = path.extname(entry.name).toLowerCase();
          if (!IGNORE_EXTENSIONS.includes(ext)) {
            structure.push(`${prefix}${currentPrefix}${entry.name}`);
  ...
  async function writeFileContents(dirPath: string) {
    const entries = await fsPromises.readdir(dirPath, { withFileTypes: true });
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        await writeFileContents(fullPath);
      } else {
        const ext = path.extname(entry.name).toLowerCase();
        ...
        try {
          ...
          writeStream.write(`### ${relativePath}\n\n`);
          writeStream.write("```" + (ext ? ext.slice(1) : '') + "\n");
          writeStream.write(content);
          writeStream.write("\n```\n\n");
        } catch (err) {
          writeStream.write(`### ${relativePath} (Failed to read)\n\n`);
        }
      }
    }
  }

  await writeFileContents(rootPath);
  writeStream.end();
}
