...
function activate(context) {
    ...
    const exportCommand = vscode.commands.registerCommand('claude-project-import-export.exportProject', async () => {
        let projectRoot;
        ...
        const confirmRoot = await vscode.window.showInformationMessage(`Selected project folder: ${projectRoot}`, 'Confirm', 'Change Folder');
    ...
    const importCommand = vscode.commands.registerCommand('claude-project-import-export.importProject', async () => {
        let importSource;
...
async function exportProjectToFile(rootPath, exportPath, progress) {
    ...
    async function generateProjectStructure(rootDir) {
        const structure = [];
        async function traverseDir(dir, prefix = '') {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                if (IGNORE_DIRS.some(ignoredDir => relativePath.split(path.sep).includes(ignoredDir))) {
                    continue;
                }
    ...
    async function traverseDirectory(dirPath) {
        ...
        for (const entry of entries) {
            ...
            if (IGNORE_DIRS.some(ignoredDir => relativePath.split(path.sep).includes(ignoredDir)) || IGNORE_FILES.includes(entry.name)) {
                continue;
            }
            if (entry.isDirectory()) {
...
async function importProjectFiles(fileContent, destinationPath, progress) {
